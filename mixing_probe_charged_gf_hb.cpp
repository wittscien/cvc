// Haobo: copied from mixing_probe_src_oet_gf_test, developing charged version
// Haobo: copied from dev-cpff-gf
/***************************************************************************
 *
    ...
    for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
    {
      ...
      for( int i = 0; i < spin_color_dilution; i++) 
      {
        ...
        if ( check_propagator_residual ) {
          check_residual_clover ( &(spinor_work[1]), &(spinor_work[0]), gauge_field_with_phase, lmzz[iflavor], 1 );
        }
...
#ifdef _GFLOW_QUDA
            _performGFlowForward ( propagator_gf[iflavor][isc], propagator_gf[iflavor][isc], &smear_param, update_gauge );
#elif defined _GFLOW_CVC
